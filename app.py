...
def log_glucose():
    ...
    if raw_val and str(raw_val).strip() != "":
        try:
            bg_val = float(raw_val)
        except ValueError:
            bg_val = glucose_history[-1]['value'] if glucose_history else 100.0
    else:
        bg_val = glucose_history[-1]['value'] if glucose_history else 100.0

    raw_carbs = data.get('carbs')
    if raw_carbs and str(raw_carbs).strip() != "":
        try:
            bg_val += (float(raw_carbs) * 0.5)
        except ValueError:
            pass

    # Robust Insulin Parsing
    raw_insulin = data.get('insulin')
    if raw_insulin and str(raw_insulin).strip() != "":
        try:
            bg_val -= (float(raw_insulin) * 2.0)
        except ValueError:
            pass
...
def analyze_vision():
    ...
    vision_prompt = """
    You are an expert clinical AI specialist.
    1. Extract the active glucose reading.
    2. Provide a HIGHLY DETAILED clinical summary (at least 150 words). Include:
       - What this specific reading means biologically.
       - The standard healthy safe zones and targets.
       - 3 highly actionable, immediate lifestyle/dietary tips to balance their blood sugar right now.

    CRITICAL JSON RULE: You must respond ONLY with valid JSON. DO NOT use literal newlines (\n) inside the summary string, as it breaks JSON parsing. Use the exact characters <br><br> for paragraph breaks.

    {
        "extracted_glucose": 130,
        "confidence_score": 0.98,
        "summary": "This reading indicates... <br><br><strong>Safe Zones:</strong> Standard fasting targets are... <br><br><strong>Actionable Tips:</strong><br>1. Drink water...<br>2. Light activity...<br>3. Monitor..."
    }
    """
    ...
    try:
            ...
            messages=[
                {
                    "role": "user",
                    "content": [
                        {"type": "text", "text": f"{vision_prompt}\n\nExtract clinical data from this image. Output strict JSON only."},
        ...
        print(f"RAW VISION OUTPUT: {raw_response}") 
        ...
        if start_idx != -1 and end_idx != -1:
            ...
            try:
                ...
                analysis_data = json.loads(clean_json, strict=False) 
            except json.JSONDecodeError:
                fallback_match = re.search(r'"extracted_glucose"\s*:\s*(\d+)', clean_json)
                val = int(fallback_match.group(1)) if fallback_match else None
                analysis_data = {
                    "extracted_glucose": val, 
                    "confidence_score": 0.85, 
                    "summary": clean_json.replace('\n', '<br>') 
                }
        else:
            analysis_data = {"extracted_glucose": None, "confidence_score": 0.0, "summary": "Failed to locate structural data within AI response."}
...
def chat_with_ai():
    ...
    - YOU MUST ALWAYS CLARIFY TO THE USER THAT YOU ARE AN AI.
    - ALWAYS ADVISE THE USER TO CONSULT A HEALTHCARE PROFESSIONAL FOR MEDICAL ADVICE AND BEFORE MAKING ANY CHANGES TO TREATMENT OR INSULIN DOSAGE.
    - NEVER PROVIDE MEDICAL DIAGNOSES OR DOSAGE ADVICE.
    ...
    Synthesize the telemetry and lab reports. Keep advice educational and safe.
    """
    try:
        ...
        print(f"\n--- GROQ API ERROR ---\n{str(e)}\n----------------------\n")
        return jsonify({"error": "AI unavailable. Check VSCode terminal."}), 500
